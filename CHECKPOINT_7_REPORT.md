...
- Verified audit-fix baseline: `424c3d2f931d29585551752100e1cb0ba39a04cf`; [passing run 35397600923](https://github.com/GlobexaTech/Globexa_CRM_Python/actions/runs/35397600923), with 393 backend, 34 unit and 73 browser tests and zero failures/errors/skips.
...
Resolved findings include normalized approval hash mismatches; lost AI usage counters after session refresh; revoked-owner termination; schedule starvation; provider-rate grouping; cross-workflow chain propagation/cycles; unsafe continuation after unknown delivery; execution self-cancellation resurrection; queued sends crossing business hours; future/branch-dependent output references; rolled-back failed-attempt accounting; stale advanced JSON after visual edits; invalid JSON validity not clearing after a visual correction; a dashboard accessibility navigation race; and an intermittent reset of a read-only test connection (one network-only GET retry, with all HTTP failures and mutations still checked normally). Regression coverage retains each relevant security or behavioral boundary.
