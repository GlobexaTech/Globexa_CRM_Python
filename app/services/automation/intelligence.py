...
async def inspect_entity(db, tenant_id, actor_id, kind, entity_type, entity_id):
    if kind == "customer":
        ...
        return await customer360(
            ...
            tenant_id,
            actor_id,
            {"lead": "leads", "contact": "contacts"}.get(entity_type, "contacts"),
            entity_id,
            30,
            0,
        )
    if kind == "lead" or (kind == "next_best_action" and entity_type == "lead"):
        ...
        activity = await db.scalar(
            ...
            .where(Activity.tenant_id == tenant_id, Activity.lead_id == row.id)
        )
        ...
        for matched, points, reason in (
            (row.contact_id is not None, 20, "Linked contact"),
            (row.company_id is not None, 15, "Linked company"),
            (bool(row.description), 10, "Recorded requirement"),
            (activity > 0, 20, "Recorded activity"),
            (
                getattr(row.status, "value", row.status) in {"qualified", "converted"},
                35,
            ...
            if matched:
                score += points
                reasons.append({"input": reason, "points": points})
        duplicates = []
        if row.contact_id:
            duplicates = [
                str(value)
        ...
        action = (
            "review_duplicates" if duplicates else "follow_up" if tasks else "create_follow_up_task"
        )
        return {
            "id": str(row.id),
            "score": score,
            ...
            "duplicate_basis": "same linked contact; review required",
            "ai_score": row.ai_score,
            "action": action,
            "reason": "Based on stored lead state, activity and open tasks",
            "confidence": None,
            "supporting_data": {
            ...
            "enrichment": {
                ...
                "provenance": "existing CRM relationships",
            },
    ...
    if kind == "deal" or (kind == "next_best_action" and entity_type == "deal"):
        ...
        entered = await db.scalar(
            select(func.max(DomainEvent.created_at)).where(
                ...
                DomainEvent.aggregate_id == str(row.id),
        ...
        risks = []
        if days >= 14:
            risks.append("No recorded stage change for at least 14 days")
        if overdue:
            risks.append("Overdue open tasks")
        if not row.owner_id:
            risks.append("No assigned owner")
        return {
            "id": str(row.id),
            "health": "at_risk" if risks else "on_track",
            "stalled": days >= 14,
            "stage_days": round(days, 2),
            "action": "review_stalled_deal"
            if days >= 14
            else "complete_overdue_tasks"
            if overdue
            else "plan_next_contact",
            "reason": "; ".join(risks) if risks else "No configured risk rule matched",
            "confidence": None,
    ...
    if kind == "campaign":
        ...
        query = (
            ...
            .where(
                CampaignRecipient.tenant_id == tenant_id, CampaignRecipient.campaign_id == row.id
            )
        )
        ...
        return {
            ...
            "conversion_limitation": "No verified campaign-to-deal conversion attribution",
            "provenance": "persisted provider recipient events",
        }
...
async def analytics(db, tenant_id, actor_id, automation_id=None):
    ...
    counts = (
        await db.execute(
            select(AutomationExecution.state, func.count())
            .where(*execution_filter)
            .group_by(AutomationExecution.state)
        )
    ).all()
    values = {state: count for state, count in counts}
    total = sum(values.values())
    terminal = sum(
        values.get(state, 0) for state in ("COMPLETED", "FAILED", "CANCELLED", "EXPIRED")
    )
    duration = await db.scalar(
        select(
            func.avg(
                func.extract(
                    "epoch", AutomationExecution.completed_at - AutomationExecution.started_at
                )
            )
        ).where(*execution_filter)
    )
    ...
    step_duration = await db.scalar(
        select(
            func.avg(
                func.extract(
                    "epoch",
                    AutomationStepExecution.completed_at - AutomationStepExecution.started_at,
                )
            )
        ).where(AutomationStepExecution.tenant_id == tenant_id, steps)
    )
    failures = (
        await db.execute(
            select(AutomationStepExecution.error_code, func.count())
            .where(
                AutomationStepExecution.tenant_id == tenant_id,
                steps,
                AutomationStepExecution.error_code.is_not(None),
            )
            .group_by(AutomationStepExecution.error_code)
    ...
    usage = (
        await db.execute(
            select(
                ...
                AIUsageLog.automation_execution_id.in_(execution_ids),
            )
        )
    ).one()
    approvals = (
        await db.execute(
            ...
            .where(
                ...
                ApprovalRequest.automation_execution_id.in_(execution_ids),
            )
    ...
    decided = sum(
        approval_counts.get(state, 0) for state in ("approved", "executed", "rejected", "failed")
    )
    provider_actions = await db.scalar(
        select(func.count())
        .select_from(AutomationStepExecution)
        .where(
            AutomationStepExecution.tenant_id == tenant_id,
            steps,
            AutomationStepExecution.state == "COMPLETED",
            AutomationStepExecution.input["action"].astext.in_(
                ["send_email", "send_whatsapp", "webhook_call"]
            ),
        )
    )
    return {
        "executions": total,
        "states": values,
        "success_rate": values.get("COMPLETED", 0) / terminal if terminal else None,
        "failure_rate": values.get("FAILED", 0) / terminal if terminal else None,
        "average_duration_seconds": float(duration) if duration is not None else None,
        "average_step_duration_seconds": float(step_duration)
        if step_duration is not None
        else None,
        "top_failures": [{"code": code, "count": count} for code, count in failures],
        "ai_calls": usage[0],
        "measured_tokens": usage[1],
        "estimated_cost_usd": usage[2] if not usage[4] else None,
        "measured_partial_cost_usd": usage[2],
        "unknown_usage_calls": usage[3],
        "unknown_cost_calls": usage[4],
        "provider_actions": provider_actions,
        "approvals": approval_counts,
        "approval_rate": (approval_counts.get("approved", 0) + approval_counts.get("executed", 0))
        / decided
        if decided
        else None,
        "conversion_rate": None,
        "conversion_limitation": "No verified conversion attribution is recorded",
    }
