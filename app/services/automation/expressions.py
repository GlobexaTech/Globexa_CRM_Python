...
FIELDS = {
    ...
    "automation": {"execution_id", "id", "version", "chain_id"},
...
def evaluate(condition, context, before=None):
    ...
    if op in {"contains", "not_contains", "starts_with", "ends_with"}:
        ...
        result = {
            ...
            "not_contains": target not in value,
            "starts_with": value.startswith(target),
            "ends_with": value.endswith(target),
        }
        return result[op]
    if (
...
def validate_templates(value, variables=(), nodes=()):
    if isinstance(value, dict):
        for item in value.values():
            validate_templates(item, variables, nodes)
    elif isinstance(value, list):
        for item in value:
            validate_templates(item, variables, nodes)
    elif isinstance(value, str):
        for match in TEMPLATE.finditer(value):
            validate_path(match[1], variables, nodes)
...
def render(value, context, *, html_destination=False):
    if isinstance(value, dict):
        return {
            key: render(item, context, html_destination=html_destination)
            for key, item in value.items()
        }
    if isinstance(value, list):
        return [render(item, context, html_destination=html_destination) for item in value]
    ...
    if whole and not html_destination:
        resolved = lookup(whole)
        if isinstance(resolved, str) and len(resolved) > 12000:
            raise HTTPException(422, "Rendered content exceeds limit")
        return resolved
