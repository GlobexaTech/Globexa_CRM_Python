...
MODELS = {"lead": Lead, "contact": Contact, "deal": Deal, "task": Task, "message": Message, "campaign": Campaign}
...
async def snapshot(db, tenant_id, actor_id, entity):
    ...
    if entity and entity.get("entity_type"):
        ...
        result[kind] = jsonable_encoder({field: getattr(row, field) for field in FIELDS[kind] if hasattr(row, field)})
        if kind == "lead":
