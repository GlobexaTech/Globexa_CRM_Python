...
class WebhookFailure(Exception):
    def __init__(self, code, *, retryable=False, uncertain=False, retry_after=0):
        self.code, self.retryable, self.uncertain, self.retry_after = (
            code,
            retryable,
            uncertain,
            retry_after,
...
def validate_destination(url):
    ...
    hosts = {
        value.strip().lower()
        for value in os.environ.get("AUTOMATION_WEBHOOK_ALLOWED_HOSTS", "").split(",")
        if value.strip()
    }
    try:
    ...
    if (
        ...
        raise HTTPException(
            422,
            "Webhook destination must be an administrator-allowed HTTPS host without credentials or query",
        )
    return parsed
...
def post(url, payload, key):
    ...
    if len(body) > 24000:
        raise HTTPException(422, "Webhook payload exceeds limit")
    ...
    try:
        ...
        if response.status in {429, 503}:
            ...
            raise WebhookFailure(
                ...
                retry_after=min(3600, int(retry)) if retry.isdigit() else 0,
            )
        if 300 <= response.status < 400:
            raise WebhookFailure("webhook_redirect_forbidden")
        if not 200 <= response.status < 300:
            raise WebhookFailure("webhook_rejected", uncertain=response.status >= 500)
        return {"status": "sent", "http_status": response.status}
    except (OSError, TimeoutError, HTTPTransportError) as exc:
