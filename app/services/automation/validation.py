...
def validate_structure(definition):
    ...
    if any(
        not re.fullmatch(r"[a-z][a-z0-9_]{0,39}", key) or "__" in key
        for key in definition.variables
    ):
    ...
    for index, node in enumerate(definition.nodes):
        ...
        if any(target not in ids for target in edges[node.id]):
            raise ValueError("Unknown graph edge")
        if node.on_error == "fallback" and not node.fallback:
            raise ValueError("Fallback requires an action node")
        if node.fallback and definition.nodes[ids.index(node.fallback)].type != "action":
            raise ValueError("Fallback must target an action")
        if node.type == "condition":
            validate_condition(node.condition, definition.variables, ids)
        elif node.type == "delay":
            if set(node.arguments) - {
                "seconds",
                "minutes",
                "hours",
                "days",
            ...
            if len(modes) != 1:
                raise ValueError("Select exactly one wait mode")
            ...
            if mode in {"seconds", "minutes", "hours", "days"}:
                ...
                if (
                    ...
                    or not 0
                    <= value * {"seconds": 1, "minutes": 60, "hours": 3600, "days": 86400}[mode]
                    <= 2592000
                ):
                ...
                if (
                    not 5 <= node.arguments.get("poll_seconds", 60) <= 3600
                    or not 1 <= node.arguments.get("timeout_seconds", 86400) <= 2592000
                ):
                    raise ValueError("Wait polling/timeout exceeds bounds")
        elif node.type in {"ai", "ai_decision"}:
            if set(node.arguments) - {"kind", "instruction", "url"}:
                raise ValueError("Unknown AI option")
            if node.arguments.get("kind", "summarize") not in AI_KINDS:
                raise ValueError("Unsupported AI action")
            if (
                not isinstance(node.arguments.get("instruction", ""), str)
                or len(node.arguments.get("instruction", "")) > 2000
            ):
                raise ValueError("AI instruction too long")
            ...
            if set(node.arguments) - {"kind", "entity_type", "entity_id"} or node.arguments.get(
                "kind"
            ) not in {"lead", "deal", "campaign", "customer", "next_best_action"}:
            ...
            for field, value in args.items():
                if isinstance(value, str) and TEMPLATE.search(value):
                    ...
                    if real.get("format") == "uuid":
                        args[field] = "00000000-0000-4000-8000-000000000001"
                    elif real.get("format") == "email":
                        args[field] = "variable@example.com"
                    elif real.get("type") == "integer":
                        args[field] = 0
                    elif real.get("type") == "boolean":
                        args[field] = False
                    else:
                        args[field] = "value"
    ...
    for node in ids:
        visit(node)
    predecessors = {key: set() for key in ids}
    for source, targets in edges.items():
        for target in targets: predecessors[target].add(source)
    reachable = set()
    def reach(key):
        if key in reachable: return
        reachable.add(key)
        for target in edges[key]: reach(target)
    reach(ids[0])
    if reachable != set(ids): raise ValueError("Every node must be reachable from the trigger")
    dominators = {key: ({key} if key == ids[0] else set(ids)) for key in ids}
    for _ in ids:
        for key in ids[1:]:
            dominators[key] = {key} | set.intersection(*(dominators[parent] for parent in predecessors[key]))
    def references(value):
        if isinstance(value, dict):
            ...
            paths = [match[1].strip() for match in TEMPLATE.finditer(value)]
            if value.startswith("steps."): paths.append(value)
            for path in paths:
                if path.startswith("steps."): yield path.split(".")[1]
    for node in definition.nodes:
        if any(ref not in dominators[node.id] - {node.id} for ref in references([node.arguments, node.condition])):
            raise ValueError("Step output must come from a preceding step on every path")
...
async def validate_chains(db, tenant_id, automation_id, definition):
    from app.models import Automation, AutomationVersion
    limits = await policy(db, tenant_id)
    examined = set()
    async def visit(value, trail):
        if len(trail) > limits.max_depth: raise HTTPException(422, "Published workflow chain exceeds depth quota")
        for node in value.nodes:
            if node.action != "start_automation": continue
            try: target = UUID(node.arguments["automation_id"])
            except (ValueError, TypeError): raise HTTPException(422, "Chained workflows require a fixed automation ID") from None
            if target in trail: raise HTTPException(422, "Published workflow chain contains a cycle")
            if target in examined: continue
            examined.add(target)
            if len(examined) > 100: raise HTTPException(422, "Published workflow chain exceeds validation quota")
            row = await owned(db, Automation, tenant_id, target)
            if row.status != "ACTIVE": raise HTTPException(422, "Chained workflow must be active")
            version = await db.scalar(select(AutomationVersion).where(AutomationVersion.tenant_id == tenant_id,
                AutomationVersion.automation_id == target, AutomationVersion.number == row.version))
            if not version: raise HTTPException(422, "Chained workflow requires a published version")
            await visit(Definition.model_validate(version.definition), [*trail, target])
    await visit(definition, [automation_id])
...
async def validate_publish(db, tenant_id, actor_id, definition):
    ...
    if len(definition.nodes) > limits.max_steps:
        raise HTTPException(422, "Workflow exceeds configured step quota")
    for integration_id in definition.credentials.values():
        ...
        if integration.status != "connected":
            raise HTTPException(422, "Referenced provider is not connected")
    for node in definition.nodes:
        if node.type in {"ai", "ai_decision"}:
            await authorize(db, tenant_id, actor_id, "ai:chat")
            if (
                limits.max_ai_calls == 0
                or limits.daily_ai_calls == 0
                or limits.monthly_ai_calls == 0
            ):
                raise HTTPException(403, "AI automation usage is disabled")
            from app.services.automation.ai import configured_gateway

            try:
                gateway = configured_gateway()
            except (RuntimeError, ValueError):
                raise HTTPException(
                    422, "Configure an approved AI provider/model before publishing"
                ) from None
            for provider in gateway.providers.values():
                await provider.client.aclose()
        if node.type not in {"action", "approval"}:
            continue
        name = "request_approval" if node.type == "approval" else node.action
        permission = (
            TOOL_SPECS[canonical(name)][0] if canonical(name) in STANDARD else EXTRA[name][0]
        )
        await authorize(db, tenant_id, actor_id, permission)
        templated = any(
            isinstance(value, str) and TEMPLATE.search(value) for value in node.arguments.values()
        )
        if not templated:
            await permitted(db, tenant_id, actor_id, name, node.arguments)
        if name in {"send_email", "send_whatsapp"}:
            if "{{" in str(node.arguments.get("conversation_id")):
                if not definition.credentials:
                    raise HTTPException(
                        422, "Dynamic sends require an explicit credential reference"
                    )
            else:
                conversation = await owned(
                    db, Conversation, tenant_id, UUID(node.arguments["conversation_id"])
                )
                if conversation.integration_id not in definition.credentials.values():
                    raise HTTPException(
                        422, "Send provider must be explicitly referenced by the version"
                    )
            for integration_id in definition.credentials.values():
                ...
                if "send" not in adapter_for(integration).capabilities:
                    raise HTTPException(422, "Referenced provider cannot send messages")
