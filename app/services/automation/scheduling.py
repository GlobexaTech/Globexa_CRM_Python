...
def cron_field(value, low, high):
    result = set()
    if len(value) > 40: raise ValueError("Cron field too long")
    for part in value.split(","):
        pieces = part.split("/")
        if len(pieces) > 2: raise ValueError("Invalid cron step")
        step = int(pieces[1]) if len(pieces) == 2 else 1
        if step < 1 or step > high - low + 1: raise ValueError("Invalid cron step")
        base = pieces[0]
        if base == "*": start, end = low, high
        elif "-" in base:
            bounds = base.split("-")
            if len(bounds) != 2: raise ValueError("Invalid cron range")
            start, end = map(int, bounds)
        else: start = end = int(base)
        if not low <= start <= end <= high: raise ValueError("Cron value out of range")
...
def parse_cron(expression):
    ...
    if len(pieces) != 5: raise ValueError("Cron requires minute, hour, day, month and weekday")
...
def next_occurrence(configuration, after):
    ...
    if after.tzinfo is None: raise ValueError("Schedule anchor must be timezone-aware")
    if spec.kind == "once": return spec.at if spec.at > after else None
    ...
    for offset in range(367):
        ...
        if spec.kind == "weekly" and day.weekday() != spec.weekday: continue
        if spec.kind == "monthly" and day.day != spec.day: continue
        if spec.kind == "cron":
        ...
        for hour in hours:
            for minute in minutes:
                ...
                if candidate.astimezone(zone).replace(tzinfo=None) != local.replace(tzinfo=None): continue
                if candidate > after: return candidate
    raise ValueError("Schedule has no occurrence within 366 days")


def business_open(configuration, instant):
    if not configuration: return True
    spec = BusinessHours.model_validate(configuration)
    if spec.allow_outside_hours: return True
    local = instant.astimezone(timezone(spec.timezone))
    return (local.weekday() in spec.weekdays and local.date().isoformat() not in spec.holidays
            and spec.start <= local.strftime("%H:%M") < spec.end)


def next_business_open(configuration, instant):
    if business_open(configuration, instant): return instant
    spec = BusinessHours.model_validate(configuration)
    zone = timezone(spec.timezone)
