...
async def validate_context(db, approval):
    execution = await owned(
        db, AutomationExecution, approval.tenant_id, approval.automation_execution_id
    )
    ...
    expected = {
        ...
        "automation_step_id": str(step.id),
    }
    if (
        execution.version_id != version.id
        or step.execution_id != execution.id
        or execution.current_node != step.node_key
        or execution.actor_id != approval.requesting_user_id
        or execution.state not in {"RUNNING", "WAITING"}
        or step.state not in {"RUNNING", "WAITING"}
        or automation.status != "ACTIVE"
        or automation.owner_id != execution.actor_id
...
async def validate_action(db, approval, approver_id):
    ...
    if (
        not hmac.compare_digest(fingerprint(action), approval.action_hash)
        or await binding(db, approval.tenant_id, name, action["arguments"]) != action["binding"]
    ):
...
async def request(db, execution, step, action, arguments):
    ...
    row = await request_approval(
        ...
        "automation:" + action,
        arguments,
        "automation-step:" + str(step.id),
        automation_context={
            "automation_execution_id": execution.id,
            "automation_version_id": execution.version_id,
